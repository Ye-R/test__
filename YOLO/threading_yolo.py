...
import time
import darknet
from YOLO.traffic_hsv import lightsParser

# Parameters
SIZE = 18  # Buffer size (if a class reaches SIZE frames -> do mission)
...
class YOLO():
    def __init__(self, db):  # db is optional
        ...
        self.sign = {key: [0, 0] for key in
                     self.definer}  # Initialize a dict for each type to store a counter and precision checker
        ...
        configPath = "C:\\Users\\HEVEN_A\\darknet\\build\\darknet\\x64\\kcity\\2020_yolov4_config.cfg"
        weightPath = "C:\\Users\\HEVEN_A\\darknet\\build\\darknet\\x64\\kcity\\2020_yolov4_config_best.weights"
        metaPath = "C:\\Users\\HEVEN_A\\darknet\\build\\darknet\\x64\\kcity\\2020_obj.data"
        ...
        if detections:
            for detection in detections:
                ...
                if area < MAX_SIZE and area > MIN_SIZE:
                    self.yolo_save_sign(detection[0])
                    cv2.rectangle(img, pt1, pt2, (0, 0, 0), 1)
                    cv2.putText(img, detection[0] + " [" + detection[1] + "]",
    ...
    def __main(self):
        ...
        # cap = self.db.main_cam.data
        # cap = cv2.VideoCapture('/dev/video2')
        # cap = cv2.VideoCapture('Main.avi')
        ...
        self.last_mission = ''
        self.TF_flag = [0, 0, 0]
        while not self.flag.system_stop:  # By stopping system, Reading GPS should be stopped, too.
            if self.flag.yolo_stop:
                time.sleep(0.1)
            else:
                prev_time = time.time()
                ret, frame_read = True, self.db.main_cam.data
                # frame_read = self.db.main_cam.data
                ...
                detections = darknet.detect_image(netMain, altNames, darknet_image, THRESH)  # Uses YOLO to detect
    ...
    def check_lights(self):
        if self.sign['red_light'][0] > SIZE:  # *Car to-do*: Reset the buffer (not left and right), find the line and stop. **add better implementation
            if self.sign['arrow_light'][0] > SIZE:
                ...
                self.reset_dict(1)
            self.db.trafficlight = self.last_mission
    ...
    def yolo_save_sign(self, data):
        if len(self.db.yolo_sign_list) > 10:
            self.db.yolo_sign_list.pop(0)
        self.db.yolo_sign_list.append(data)
        self.yolo_find_current_sign()

    def yolo_find_current_sign(self):
        yolo_cnt_dict = {}
        for sign in self.db.yolo_sign_list:
            if sign not in yolo_cnt_dict:
                yolo_cnt_dict[sign] = 1
                continue
            yolo_cnt_dict[sign] += 1
        self.db.yolo_buffer_sign = max(yolo_cnt_dict, key=yolo_cnt_dict.get)
