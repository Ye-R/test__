...
class YOLO():
    def __init__(self):
        ...
        configPath = "pams19/yolov3-t4.cfg"
        weightPath = "backup/backup-pams/yolov3_final.weights"
        metaPath = "pams19/obj.data"
    ...
    def main(self):
        ...
        while True:
            ...
            self.high_vals = {k: v for (k, v) in self.sign.items() if
                              v[0] >= SIZE}  # Check if there are any values above size limit

            if self.high_vals:  # If there is any sign > SIZE
                max_sign = max(self.high_vals, key=lambda key: self.high_vals[key][
                    0])  # Finds the key with highest frame_count on the dictionary

                self.trafficlight = max_sign
            ...
            cv2.putText(image, 'Mission: ' + self.trafficlight, (10, 400), font, fontScale, fontColor, lineType)
